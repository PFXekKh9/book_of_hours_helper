...
# We need to track the "Level" of the recipe.
# The wiki groups tables by "Prentice-level", "Scholar-level", "Keeper-level".
# We can find each table and check its caption or previous headers.
...
for table in soup.find_all('table', class_='fandom-table'):
    ...
    level_val = ""
    if caption:
        cap_text = caption.text.lower()
        if "prentice" in cap_text:
            level_name = "徒弟(レベル5)"
            level_val = "5"
        elif "scholar" in cap_text:
            level_name = "学者(レベル10)"
            level_val = "10"
        elif "keeper" in cap_text:
            level_name = "番人(レベル15)"
            level_val = "15"
    ...
    for tr in table.find_all('tr'):
        ...
        if len(tds) >= 6:
            ...
            # col 3/4 is principles (we can extract from the text, but level is usually enough)
            # col 5 is requirements (Additional Requirements)
            reqs = []
            req_a = tds[5].find('a')
            if req_a:
            ...
            recipe_str = f"クラフト: {level_name} （必要なスキル: {skills_str}）"
...
for item in items_data['items']:
    ...
    # Remove ALL previous craft strings from how_to_get
    cleaned_htg = []
    ...
    # Prepend the newly generated accurate recipes
...
print("Regenerated precise crafting recipes for all items!")
